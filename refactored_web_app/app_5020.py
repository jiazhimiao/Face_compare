...
from core.visualization_v6 import create_dual_result_visual_v6, create_single_result_visual_v6
...
UI_VERSIONS = {
    "upgrade_port_5020": "http://127.0.0.1:5020",
...
def detect_face():
    ...
    visual_path = create_single_result_visual_v6(
        title="\u4eba\u8138\u68c0\u6d4b\u7ed3\u679c",
        ...
        output_path=RUNTIME_OUTPUT_DIR / f"detect_result_5020_{g.request_id}.png",
    )
    payload = {
    ...
    write_audit_event({**request_summary(), "operation": "detect_face", "result": payload["face_detection"], "ui_version": "5020"})
...
def verify_identity():
    ...
    visual_path = create_dual_result_visual_v6(
        title="\u8eab\u4efd\u6838\u9a8c\u7ed3\u679c",
        ...
        output_path=RUNTIME_OUTPUT_DIR / f"verify_result_5020_{g.request_id}.png",
    )
    payload = {
    ...
    write_audit_event({**request_summary(), "operation": "verify_identity", "result": payload["identity_verification"], "ui_version": "5020"})
    return success_response(payload, result.message)
...
def check_blacklist():
    ...
    visual_path = create_dual_result_visual_v6(
        ...
        output_path=RUNTIME_OUTPUT_DIR / f"blacklist_result_5020_{g.request_id}.png",
    )
    payload = {
    ...
    write_audit_event({**request_summary(), "operation": "check_blacklist", "result": payload["blacklist_check"], "ui_version": "5020"})
    return success_response(payload, result.message)
