...
def _draw_labeled_image_panel(draw: ImageDraw.ImageDraw, canvas: Image.Image, image_path: str | Path, panel_box: tuple[int, int, int, int], label: str, bbox: list[int] | None, accent: tuple[int, int, int], title_font, note_font) -> None:
    ...
    draw.text((x1 + 24, y1 + 58), "\u56fe\u50cf\u533a\u57df\u4fdd\u7559\u5b89\u5168\u8fb9\u8ddd\uff0c\u6807\u51c6\u5316\u5c55\u793a\u4eba\u8138\u6846\u4f4d\u7f6e\u3002", font=note_font, fill=MUTED)
    image = Image.open(image_path)
    inner_box = (x1 + 24, y1 + 100, x2 - 24, y2 - 24)
...
def create_single_result_visual_v6(title: str, image_path: str | Path, bbox: list[int] | None, status_label: str, status_tone: str, metrics: list[tuple[str, str]], output_path: str | Path) -> Path:
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    title_font, body_font, note_font, mini_font = _fonts()
    ...
    draw.rounded_rectangle((24, 24, 1576, 1036), radius=36, fill=SURFACE_ALT, outline=BORDER)
    draw.text((60, 56), title, font=title_font, fill=TEXT)
    _draw_status_pill(draw, 60, 124, status_label, mini_font, status_tone)
    draw.text((60, 182), "\u7ed3\u679c\u56fe\u91c7\u7528\u7edf\u4e00\u89c6\u89c9\u7cfb\u7edf\uff0c\u7a81\u51fa\u7ed3\u8bba\uff0c\u6307\u6807\u4e0e\u56fe\u50cf\u5bf9\u7167\u3002", font=note_font, fill=MUTED)

    image_panel_top = 248
    image_panel_bottom = 980
    image_inner_height = (image_panel_bottom - image_panel_top) - 124
    ...
    summary_box = (image_panel[2] + 32, 248, 1540, 980)

    _draw_labeled_image_panel(draw, canvas, image_path, image_panel, "\u539f\u59cb\u68c0\u6d4b\u56fe\u7247", bbox, PRIMARY, body_font, mini_font)
    draw.rounded_rectangle(summary_box, radius=28, fill=SURFACE, outline=BORDER)
    draw.text((summary_box[0] + 32, 278), "\u7ed3\u679c\u6458\u8981", font=body_font, fill=TEXT)
    draw.text((summary_box[0] + 32, 320), "\u6309\u7ed3\u8bba\u3001\u6307\u6807\u3001\u5904\u7406\u5efa\u8bae\u7684\u5c42\u6b21\u6392\u5217\uff0c\u4fbf\u4e8e\u5f53\u573a\u6f14\u793a\u548c\u4ea4\u4ed8\u3002", font=mini_font, fill=MUTED)
    _draw_metric_cards(draw, metrics, (summary_box[0] + 32, 374), summary_box[2] - summary_box[0] - 64, body_font, mini_font)

    canvas.save(output_path, format="PNG", compress_level=0)
    return output_path


def create_dual_result_visual_v6(title: str, left_image_path: str | Path, right_image_path: str | Path, left_bbox: list[int] | None, right_bbox: list[int] | None, left_label: str, right_label: str, status_label: str, status_tone: str, metrics: list[tuple[str, str]], output_path: str | Path) -> Path:
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    title_font, body_font, note_font, mini_font = _fonts()
    width = 1760
    draft = Image.new("RGB", (width, 1600), BG)
    draft_draw = ImageDraw.Draw(draft)
    metrics_height = _measure_metric_cards_height(draft_draw, metrics, 1528, body_font, mini_font)
    summary_top = 792
    summary_metrics_top = 914
    summary_bottom = max(1140, summary_metrics_top + metrics_height + 40)
    ...
    draw.text((60, 56), title, font=title_font, fill=TEXT)
    _draw_status_pill(draw, 60, 124, status_label, mini_font, status_tone)
    draw.text((60, 182), "\u53cc\u56fe\u5bf9\u7167\u7ed3\u6784\u5c06\u539f\u56fe\u3001\u4eba\u8138\u6846\u548c\u5173\u952e\u6307\u6807\u653e\u5728\u540c\u4e00\u89c6\u56fe\u91cc\uff0c\u63d0\u9ad8\u6f14\u793a\u6548\u7387\u3002", font=note_font, fill=MUTED)

    _draw_labeled_image_panel(draw, canvas, left_image_path, (60, 248, 842, 760), left_label, left_bbox, PRIMARY, body_font, mini_font)
    _draw_labeled_image_panel(draw, canvas, right_image_path, (878, 248, 1660, 760), right_label, right_bbox, SUCCESS, body_font, mini_font)

    summary_box = (60, summary_top, 1660, summary_bottom)
    draw.rounded_rectangle(summary_box, radius=28, fill=SURFACE, outline=BORDER)
    draw.text((96, 824), "\u5173\u952e\u7ed3\u679c\u4e0e\u6307\u6807", font=body_font, fill=TEXT)
    draw.text((96, 866), "\u7edf\u4e00\u6807\u7b7e\uff0c\u7edf\u4e00\u5361\u7247\uff0c\u7edf\u4e00\u5bf9\u6bd4\u8bed\u4e49\u3002", font=mini_font, fill=MUTED)
